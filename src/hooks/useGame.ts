...
export function useGame() {
  ...
  // Skip prison (after watching ad)
  const skipPrison = useCallback(async () => {
    const updatedPlayer = {
      ...player,
      prisonTime: null,
    };

    await saveGameMutation.mutateAsync(updatedPlayer);
    setInPrison(false);
  }, [player, saveGameMutation]);
  ...
  return {
    ...
    executeMission,
    skipPrison,
    availableMissions: missions.map((mission) => ({
      ...mission,
